module main (
    ...
    reg [4:0] kilitli_kelime;
    reg [2:0] kilitli_ekran;

    always @(posedge clk) begin
        if (falling_edge) begin
            kilitli_kelime <= letter_selection; // Sinyali gördüğün an seçimi hafızaya al
            kilitli_ekran  <= display_section;  // Sinyali gördüğün an ekranı hafızaya al
        end
    end
    ...
    letter_selection letter_motor (
        .letter_id(kilitli_kelime),      // Şalterden gelen 5 bitlik sayıyı sözlüğe ver
    ...
    display_section distributor_motor (
        .letter_selection(kilitli_kelime),     // Şalterden kelime durumunu dinle
        .display_section(kilitli_ekran),   // Şalterden hedef ekranı dinle
